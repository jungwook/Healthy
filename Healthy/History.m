...
@interface History()
@property (nonatomic, strong) NSURL *workoutsPath;
@property (nonatomic, strong) NSURL *historyPath;
...
- (instancetype) init
{
    self = [super init];
    if (self) {
        self.workoutsPath = [[[[[NSFileManager defaultManager] URLsForDirectory:NSDocumentDirectory inDomains:NSUserDomainMask] lastObject] URLByAppendingPathComponent:@"Files"] URLByAppendingPathExtension:@"Workouts"];
        self.historyPath = [[[[[NSFileManager defaultManager] URLsForDirectory:NSDocumentDirectory inDomains:NSUserDomainMask] lastObject] URLByAppendingPathComponent:@"Files"] URLByAppendingPathExtension:@"History"];
...
{
    self.history = [NSMutableArray arrayWithContentsOfURL:self.historyPath];
    NSLog(@"Loaded %ld history items", self.history.count);
}

+ (void)addWorkoutToHistory:(id)workout
{
    [[History new] addWorkoutToHistory:workout];
}
...
- (void) addWorkoutToHistory:(id)workout
{
    ...
    [self.history addObject:workout];
    self.history = [self sortedHistory];
    BOOL ret = [self.history writeToURL:self.historyPath atomically:YES];
    
    NSLog(@"%@SUCCESSFULLY WROTE HISTORY:%ld ITEMS", ret ? @"" : @"UN", self.history.count);
}

- (NSMutableArray*) sortedWorkouts
{
    NSSortDescriptor *sd = [NSSortDescriptor sortDescriptorWithKey:@"Workout" ascending:YES];
    return [NSMutableArray arrayWithArray:[self.workouts sortedArrayUsingDescriptors:@[sd]]];
}

- (NSMutableArray*) sortedHistory
{
    NSSortDescriptor *sd = [NSSortDescriptor sortDescriptorWithKey:@"Date" ascending:YES];
    return [NSMutableArray arrayWithArray:[self.history sortedArrayUsingDescriptors:@[sd]]];
}

- (void) generateRandomHistoryItems
{
...
{
    NSData *data = [NSData dataWithContentsOfURL:self.workoutsPath];
    self.workouts = [NSKeyedUnarchiver unarchiveObjectWithData:data];
//    self.workouts = nil;
    if (!self.workouts) {
        self.workouts = [NSMutableArray arrayWithArray:@[
        ...
        NSSortDescriptor *sd = [NSSortDescriptor sortDescriptorWithKey:@"Workout" ascending:YES];
        self.workouts = [NSMutableArray arrayWithArray:[self.workouts sortedArrayUsingDescriptors:@[sd]]];
        NSData *data = [NSKeyedArchiver archivedDataWithRootObject:self.workouts];
        NSError *error;
        BOOL ret = [data writeToURL:self.workoutsPath options:NSDataWritingAtomic error:&error];
        if (error) {
            NSLog(@"ERROR:%@", [error localizedDescription]);
        }
        NSLog(@"%@SUCCESSFULLY WROTE WORKOUTS TO FILE", ret == NO ? @"UN" : @"");
    }
    else {
        NSSortDescriptor *sd = [NSSortDescriptor sortDescriptorWithKey:@"Workout" ascending:YES];
        self.workouts = [NSMutableArray arrayWithArray:[self.workouts sortedArrayUsingDescriptors:@[sd]]];
        NSLog(@"SUCCESSFULLY READ WORKOUTS FROM FILE:%ld Workouts", self.workouts.count);
    }
}
